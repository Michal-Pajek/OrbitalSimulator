...
namespace localization
{
	...
	{
		const std::string key{ text_id_entries::toLocalizationKey(textId) };
	...
	void LocalizationManager::applyLanguage(const Language language)
	{
		if (!m_languageApplied || m_currentLanguage != language) {
			auto newMap{ json::loadLocalizationData(getLanguageCode(language)) };
	...
	{
		json::saveLanguageCode(getLanguageCode(language));
	}
	...
	{
		try {
			const auto languageCode{ json::loadLanguageCode() };
